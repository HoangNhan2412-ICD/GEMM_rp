...
## DSP experiment flow setup

- Thêm `scripts/run_dsp_experiments.tcl` để synthesize riêng các top `PE`, `PE_array`, `MM`, `MM_ultra`, `GEMM_TOP` trên part `xck26-sfvc784-2LV-c`.
- Thêm wrapper Linux/Windows `scripts/run_dsp_experiments.sh` và `scripts/run_dsp_experiments.bat`; Windows dùng Vivado tại `D:\Vivado\2022.2\bin\vivado.bat` và báo lỗi rõ ràng nếu thiếu Vivado.
- Thêm `scripts/parse_dsp_experiments.py` để tổng hợp LUT/FF/BRAM/DSP/WNS/TNS và probe số cell DSP/multiplier cho từng top.
- Thêm `docs/dsp_experiment_report.md` làm báo cáo so sánh experiment; hiện chưa có report Vivado thật nên bảng experiment là `Needs verification`.
- Thêm `docs/dsp_experiment_plan.md` để mô tả mục tiêu, top cần synthesize, metric cần parse, giả thuyết cần kiểm chứng và quy tắc không dùng `use_dsp` trong RTL chính.
- Không sửa RTL chính và không dùng `(* use_dsp = "yes" *)` trong flow này.
